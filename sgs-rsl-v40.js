/* SGS RSL Parser V4.2.3 Official - metadata auto-fill engine */
(()=>{'use strict';
...
function parse(t){const r=reportNo(t),known=knownItems(t,r);if(known.length)return known;return unique([...parseBisphenolsV42(t),...generic(t)])}
  ...
  try{
    ...
    if(msg){
      ...
      msg.textContent=failedItems.length
        ?`V4.2.3 completed for ${r||'this report'}. ${failedItems.length} failed item(s) found and ${filled}/5 metadata fields filled. Verify against the PDF.`
        :`V4.2.3 found no verified failed result in ${r||'this report'}. ${filled}/5 metadata fields filled. Add failed items manually.`;
      msg.classList.remove('hidden');
    }
  }catch(e){
    console.error('SGS RSL V4.2.3',e);
...
window.SGSRSLV423={parse,parseBisphenolsV42,reportNo,extractMetadata,knownItems};
window.SGSRSLV422=window.SGSRSLV423;
window.SGSRSLV421=window.SGSRSLV423;
window.SGSRSLV42=window.SGSRSLV423;
