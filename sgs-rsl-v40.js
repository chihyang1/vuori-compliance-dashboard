/* SGS RSL Parser V4.2.1 Official - metadata auto-fill engine */
(()=>{'use strict';
  ...
  try{
    ...
    if(msg){
      ...
      msg.textContent=failedItems.length
        ?`V4.2.1 completed for ${r||'this report'}. ${failedItems.length} failed item(s) found and ${filled}/5 metadata fields filled. Verify against the PDF.`
        :`V4.2.1 found no verified failed result in ${r||'this report'}. ${filled}/5 metadata fields filled. Add failed items manually.`;
      msg.classList.remove('hidden');
    }
  }catch(e){
    console.error('SGS RSL V4.2.1',e);
...
window.SGSRSLV421={parse,parseBisphenolsV42,reportNo,extractMetadata};
window.SGSRSLV42=window.SGSRSLV421;
