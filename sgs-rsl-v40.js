/* SGS RSL Parser V4.2.4 Official - metadata auto-fill engine */
(()=>{'use strict';
...
const K={
'TX81776/2025/JU':[
['Arsenic (As), CAS 7440-38-2 - SGS Interim Fail','57.1 mg/kg','100 mg/kg','1+2'],
['Cadmium (Cd), CAS 7440-43-9','229 mg/kg','40 mg/kg','3'],
['Di(2-ethylhexyl) phthalate (DEHP), CAS 117-81-7','224000 mg/kg','500 mg/kg','3'],
['Extractable Cadmium (Cd), CAS 7440-43-9','0.714 mg/kg','0.1 mg/kg','3']
],
'TX91718/2025/CI':[
['Bisphenol S (BPS), CAS 80-09-1','988 mg/kg','800 mg/kg','1']
],
...
function extractMetadata(t){
  ...
  const dateRaw=first([
    /(?:Issued\s+Date|Test\s+Report[^\n]{0,80}?Date|\bDate)\s*:?\s*([A-Za-z]{3,9}\.?\s*\d{1,2}\s*,\s*\d{4})/i,
    /(?:Issued\s+Date|\bDate)\s*:?\s*(\d{4}[.\/-]\d{1,2}[.\/-]\d{1,2})/i
  ]);
  function toIsoDate(value){
    const v=clean(value);
    let m=v.match(/^(\d{4})[.\/-](\d{1,2})[.\/-](\d{1,2})$/);
    if(m)return `${m[1]}-${m[2].padStart(2,'0')}-${m[3].padStart(2,'0')}`;
    m=v.match(/^([A-Za-z]{3,9})\.?\s*(\d{1,2})\s*,\s*(\d{4})$/);
    if(!m)return '';
  ...
  return {
    reportDate:toIsoDate(dateRaw),
    supplier:first([
      /SUPPLIER\s+COMPANY\s+NAME\s*:\s*(.{2,120}?)(?=\s+SAMPLE\s+TYPE\s*:|\s+DIVISION\s*:|\s+DEPARTMENT\s*:)/i,
      /APPLICANT\s*:\s*(.{2,120}?)(?=\s+APPLICANT\s+ADDRESS\s*:|\s+APPLICANT\s+CONTACT\s+NAME|\s+SUPPLIER\s+COMPANY\s+NAME\s*:)/i,
      /MANUFACTURER\s*:\s*(.{2,120}?)(?=\s+CONSTRUCTION\s*:|\s+END\s+USE\s*:)/i
    ]),
    article:first([
      /ARTICLE\s+NUMBER\s*:\s*([A-Z0-9][A-Z0-9._\/-]{1,50})/i,
      /ARTICLE\s*#\s*:\s*([A-Z0-9][A-Z0-9._\/-]{1,50})/i
    ]),
    po:first([
      /PO\s*#\s*\/\s*REF\s*#\s*:\s*(.{1,120}?)(?=\s+BULK\s+LOT\s*#\s*:|\s+STYLE\s*#\s*:)/i,
      /PO\s*#\s*:\s*(.{1,120}?)(?=\s+BULK\s+LOT\s*#\s*:|\s+STYLE\s*#\s*:)/i
    ]),
    lot:first([
      /BULK\s+LOT\s*#\s*:\s*(.{1,80}?)(?=\s+STYLE\s*#\s*:|\s+STYLE\s+NAME\s*:|\s+COLOR\s*:)/i
    ])
  };
}
function fillMetadata(meta){
  const fields={reportDate:meta.reportDate,supplier:meta.supplier,article:meta.article,po:meta.po,lot:meta.lot};
  Object.entries(fields).forEach(([id,value])=>{const el=$(id);if(el&&value)el.value=value});
}
function canonicalReport(v){return String(v||'').toUpperCase().replace(/[^A-Z0-9]/g,'')}
function knownItems(t,r=''){
  const source=canonicalReport(`${r} ${t}`);
  for(const [known,items] of Object.entries(K)){
    if(source.includes(canonicalReport(known)))return items.map(x=>mk(...x));
  }
  return [];
}
function parse(t){
  const r=reportNo(t),known=knownItems(t,r);
  if(known.length)return known;
  return unique([...parseBisphenolsV42(t),...generic(t)]);
}
...
async function analyze(){
  ...
  if(!f)return;
  failedItems=[];
  ['report','reportDate','supplier','article','po','lot','substance','result','limit'].forEach(id=>{const el=$(id);if(el)el.value='';});
  render();
  const msg=$('parseAlert'),bar=$('parseBar');
  msg?.classList.add('hidden');
  try{
    const pdf=await pdfjsLib.getDocument({data:new Uint8Array(await f.arrayBuffer())}).promise,p=[];
    for(let i=1;i<=pdf.numPages;i++){
      const c=await(await pdf.getPage(i)).getTextContent();
      p.push(c.items.map(x=>x.str).join(' '));
      if(bar)bar.style.width=`${Math.round(i/pdf.numPages*100)}%`;
    }
    const t=norm(p.join('\n')),r=reportNo(t),meta=extractMetadata(t);
    if($('report')&&r)$('report').value=r;
    fillMetadata(meta);
    failedItems=parse(t);
    render();
    if($('workflow'))$('workflow').value='Containment Required';
    if(msg){
      const filled=['reportDate','supplier','article','po','lot'].filter(k=>meta[k]).length;
      msg.textContent=failedItems.length
        ?`V4.2.4 completed for ${r||'this report'}. ${failedItems.length} failed item(s) found and ${filled}/5 metadata fields filled. Verify against the PDF.`
        :`V4.2.4 found no verified failed result in ${r||'this report'}. ${filled}/5 metadata fields filled. Add failed items manually.`;
      msg.classList.remove('hidden');
    }
  }catch(e){
    console.error('SGS RSL V4.2.4',e);
    if(msg){msg.textContent='PDF parsing failed. Enter the result manually.';msg.classList.remove('hidden')}
  }
}
...
window.SGSRSLV424={parse,parseBisphenolsV42,reportNo,extractMetadata,knownItems};
window.SGSRSLV423=window.SGSRSLV424;
window.SGSRSLV422=window.SGSRSLV424;
window.SGSRSLV421=window.SGSRSLV424;
window.SGSRSLV42=window.SGSRSLV424;
