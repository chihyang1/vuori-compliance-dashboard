...
(() => {
  ...
  function findRequirement(text, exactLabel) {
    const requirementBlock = clean(text).match(/Requirement\s*:[\s\S]{0,1500}?(?=Remarks?\s*:|Appendix|End of Report|$)/i)?.[0] || clean(text);
    const escaped = exactLabel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex = new RegExp(`(?:^|\\s)${escaped}\\s+(?:\\d+(?:\\.\\d+)?\\s*ppb\\s*)?\\((\\d+(?:\\.\\d+)?)\\s*mg\\/kg\\)|(?:^|\\s)${escaped}\\s+(\\d+(?:\\.\\d+)?)\\s*mg\\/kg`, 'i');
    const match = requirementBlock.match(regex);
  ...
  function install() {
    ...
    if (analyze) analyze.onclick = async function(event) {
      ...
      } catch(error){ console.error('SGS RSL V3.2E parser error',error); }
    };
